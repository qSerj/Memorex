...
Этот документ не является продуктовым roadmap или окончательной архитектурой. Это карта технических
идей, накопленных до и после продуктового поворота. Канонический продукт — обычные локальные Notes
с необязательным AI, описанные в [`00_PROJECT_CONTEXT.md`](00_PROJECT_CONTEXT.md). Ни graph, ни
claims, ни generated Wiki не должны становиться обязательным посредником между пользователем и его
заметкой.

## Текущая архитектурная рамка

```text
                     ┌── manual Notes / notebooks / attachments / FTS
Capture → originals ─┼── optional AI proposals → Review → Notes
                     ├── explicit-context Discussions
                     └── future Commitments → Attention
```

SQLite хранит реестр, версии, очередь и пользовательскую организацию. Markdown остаётся переносимым
и читаемым форматом содержания заметок. Content-addressed objects хранят неизменные originals.
Производные индексы и vault можно перестроить.

Архитектурные идеи ниже применяются выборочно, когда простая модель перестаёт решать конкретную
задачу.
