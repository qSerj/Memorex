...
Memorex используется в быту, а наблюдения накапливаются в локальном `USAGE_LOG.md`. Небольшие
улучшения существующих сценариев можно делать сразу. Большие подсистемы вроде Commitments и
reminder engine требуют повторяющейся наблюдаемой боли, а потеря или порча данных и сломанный уже
обещанный сценарий всегда имеют приоритет.
...
1. Пользоваться заметками, вложениями, поиском и Discussions на реальных материалах.
2. Записывать в `USAGE_LOG.md` ожидаемое и фактическое поведение, обходной путь и число повторов.
3. Небольшие понятные улучшения существующих сценариев делать короткими итерациями.
4. Периодически разбирать журнал и выбирать один следующий более крупный эксперимент.
